...
class WaifuIM(commands.Cog):
    ...
    async def help(self, ctx):
            ...
            async with self.session.get(url) as response:
                ...
                embed = discord.Embed(title='Tag Help', description='Here is a list of available tags from the waifu.im api.\nThe `[p]tag` and `[p]ntag` commands are the only ones that require a tag. All other commands do not accapt a tag.', timestamp=datetime.timestamp())
...
#                        artist_name = image['artist']['name']
                ...
                embed = discord.Embed(timestamp=datetime.timestamp())
